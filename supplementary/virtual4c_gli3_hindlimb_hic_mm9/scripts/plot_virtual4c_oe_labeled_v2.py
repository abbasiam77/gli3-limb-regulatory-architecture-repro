...
def main():
    ap = argparse.ArgumentParser(description="Plot Virtual4C OE(KR) curve + enhancer vertical lines with staggered labels.")
    ...
    ap.add_argument("--intronic", nargs="*", default=None,
                    help="Override intronic set (green). If not provided, uses default set.")
    ...
    exclude_set = set(args.exclude) if args.exclude is not None else set()
    ...
    feats = [f for f in read_bed_features(args.bed, args.start, args.end) if f["name"] not in exclude_set]

    # Wider figure helps labeling
    ...
    ax.text(vp_mb, ymax * 1.02, "Promoter region", rotation=90, va="bottom", ha="right", fontsize=9)

    # Enhancers: vertical lines + staggered labels for close clusters
    ...
    # If two enhancers are closer than 20 kb, stagger label tiers and alternate x-offset
    close_thresh_mb = 0.020  # 20 kb
    label_tiers = [ymax * 0.16, ymax * 0.22, ymax * 0.28, ymax * 0.34]  # more tiers for wide plot
    ...
    for f in feats:
        ...
        if prev_x is not None and abs(xm - prev_x) < close_thresh_mb:
            close_run += 1
        else:
            close_run = 0

        tier = close_run % len(label_tiers)
        # Alternate x offset in points for close features
        xoff = 0
        if close_run > 0:
            xoff = 12 if (close_run % 2 == 1) else -12
