...
def main():
    ...
    fig, axes = plt.subplots(3, 1, figsize=(13, 11), dpi=150)
    ...
    axes[1].set_yticklabels([""]*len(df))
    axes[1].invert_yaxis()
    axes[1].set_title("H3K27ac (WT hindlimb E11.5, mm9)")
    axes[1].set_xlabel("mean bigWig signal")

    axes[2].barh(y, df["H3K4me3_mean"], color=colors)
    axes[2].set_yticks(list(y))
    axes[2].set_yticklabels([""]*len(df))
    axes[2].invert_yaxis()
    axes[2].set_title("H3K4me3 (WT hindlimb E11.5, mm9)")
    axes[2].set_xlabel("mean bigWig signal")
